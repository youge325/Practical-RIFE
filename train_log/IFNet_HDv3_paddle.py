...
def conv_bn(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1):
    ...
    return nn.Sequential(
        nn.Conv2D(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                  padding=padding, dilation=dilation, bias_attr=False),
...
class Head(nn.Layer):
    ...
    def __init__(self):
        ...
        self.cnn3 = nn.Conv2DTranspose(16, 4, 4, 2, 1)
...
class ResConv(nn.Layer):
    ...
    def __init__(self, c, dilation=1):
        ...
        self.beta = paddle.create_parameter((1, c, 1, 1), dtype='float32', default_initializer=paddle.nn.initializer.Constant(1.0))
        self.relu = nn.LeakyReLU(0.2, True)

    def forward(self, x):
        return self.relu(self.conv(x) * self.beta + x)

class IFBlock(nn.Layer):
    ...
    def __init__(self, in_planes, c=64):
        ...
        self.lastconv = nn.Sequential(
            nn.Conv2DTranspose(c, 4*13, 4, 2, 1),
            nn.PixelShuffle(2)
        )
...
class IFNet(nn.Layer):
    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, fastmode=True, ensemble=False):
        ...
        for i in range(5):
            if flow is None:
                # 第一次：无初始光流，直接输入基础特征
                flow, mask, feat = block[i](paddle.concat((img0[:, :3], img1[:, :3], f0, f1, timestep), 1), None, scale=scale_list[i])
                if ensemble:
                ...
                fd, m0, feat = block[i](paddle.concat((warped_img0[:, :3], warped_img1[:, :3], wf0, wf1, timestep, mask, feat), 1), flow, scale=scale_list[i])
                if ensemble:
                    print("warning: ensemble is not supported since RIFEv4.21")
                else:
                    mask = m0
